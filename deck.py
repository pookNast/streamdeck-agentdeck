...
NWS_USER_AGENT = "streamdeck-agentdeck (forgejo:pook/streamdeck-agentdeck)"
...
# Seeded from cfg.monitor_hosts at startup (see main entrypoint).
_host_status = {}
_host_status_ts = 0.0
...
def _prune_dead(sessions):
    ...
    for s in sessions:
        ...
        if not konsole_pid:
            continue
        if _terminal_backend is not None:
            alive = _terminal_backend.is_window_alive(int(konsole_pid))
        else:
            alive = None
    ...
    for s in sessions:
        ...
        if _terminal_backend is not None and svc not in _dbus_live:
            kids = _terminal_backend.get_live_dbus_kids(svc)
            _dbus_live[svc] = (kids is not None, kids or set())
        elif svc not in _dbus_live:
...
def _voice_toggle(sess):
    ...
    # Are we currently recording? (PID file exists on remote host)
    ...
    # Recording — stop + transcribe (konsole-send will fail harmlessly on the remote host;
...
# Terminal backend instance (set in main()). Delegates D-Bus/X11 operations.
...
def _konsole_services():
    if _terminal_backend and hasattr(_terminal_backend, "_konsole_services"):
        return _terminal_backend._konsole_services()
    return []

def _focused_konsole():
    if _terminal_backend and hasattr(_terminal_backend, "_focused_konsole"):
        return _terminal_backend._focused_konsole()
    return None

def _agentdeck_konsole():
    if _terminal_backend and hasattr(_terminal_backend, "_agentdeck_konsole"):
        return _terminal_backend._agentdeck_konsole()
    return None

def _any_konsole():
    if _terminal_backend and hasattr(_terminal_backend, "_any_konsole"):
        return _terminal_backend._any_konsole()
    return None

def _session_list(svc):
    if _terminal_backend and hasattr(_terminal_backend, "_session_list"):
        return _terminal_backend._session_list(svc)
    return []

def _run_in_session(svc, sid, cmd):
    if _terminal_backend and hasattr(_terminal_backend, "_run_in_session"):
        _terminal_backend._run_in_session(svc, sid, cmd)
...
def place_konsole(cmd, mode, sid=None, tmux=None):
    """Delegate to the terminal backend. Falls back to _new_window for 'window' mode."""
    if _terminal_backend is not None:
        _terminal_backend.place(cmd, mode, sid=sid, tmux=tmux)
        return
    # Fallback for non-Konsole backends: just open a new window.
...
def spawn(tool, mode):
    ...
    # overriding it back to the folder name (e.g. the launch dir's basename).
...
def render_touchscreen(deck):
    ...
    if _ui_mode == "tool":
        ...
        if _lcd_mode == "laputa":
            ...
            beach = ghibli_beach.render_beach_panorama(_anim_phase, _weather_snapshot(), _hour)
            img.paste(beach, (0, 0))
            d = ImageDraw.Draw(img)
            txt_c = TXT_BANNER
        else:
...
def main():
    ...
    if IS_XL:
        ...
        # LCD panorama default — "beach" boots into the live-weather scene.
    ...
    # Create terminal backend based on config
    from core.terminal import create_backend
    _terminal_backend = create_backend(
        cfg.terminal_mode,
        win_map=_win_map,
        save_win_map_fn=_save_win_map,
        dbus_map=_dbus_map,
        save_dbus_map_fn=_save_dbus_map,
    )
    ...
    _last_input = time.monotonic()
    # Seed host-status dict from config so the ping loop + touchscreen dots
    # have something to iterate. Empty list = monitoring thread no-ops.

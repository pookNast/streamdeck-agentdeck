...
"""
import os, re, subprocess, shutil, time, signal, signal
...
class KonsoleDBus(TerminalBackend):
    ...
    def __init__(self, win_map, save_win_map_fn, dbus_map, save_dbus_map_fn):
    ...
    def _clients_on(self, tmux):
        """Set of tmux client names attached to session `tmux`."""
        if not tmux:
            return set()
        r = subprocess.run(
    ...
    def _tmux_client_count(self):
        """Total attached tmux clients across all sessions."""
        r = subprocess.run(["tmux", "list-clients"], capture_output=True, text=True, timeout=4)
        return len(r.stdout.splitlines()) if r.returncode == 0 else 0

    def _close_session(self, svc, ksid):
        """Tear down an orphan split pane.
        SIGTERM foreground until shell is foreground, then `exit`, then verify."""
        if not ksid:
            return
        ...
        time.sleep(0.3)
        if ksid in self._session_list(svc):
            print("close_session: pane %s in %s did not close" % (ksid, svc))

    def place(self, cmd, mode, sid=None, tmux=None):
        """Place cmd in konsole using the given mode.

        Full split-verification logic: after creating a split, confirms that a
        tmux client actually bound to the new pane. If the split didn't hold,
        closes the orphan pane and falls back to a new window.
        """
        ...
        for _ in range(25):
            ...
            after_current = _qdbus(svc, "/Windows/1",
                                   "org.kde.konsole.Window.currentSession").strip()
            fresh = set(self._session_list(svc)) - before
        ...
        def _bound():
            if tmux:
                return bool(self._clients_on(tmux) - c0)
            return self._tmux_client_count() > g0
        ...
        for _ in range(50):
            ...
            self._close_session(svc, ksid)
            self._new_window(cmd, sid=sid)
            return True

        time.sleep(0.8)
        if _bound() and ksid in self._session_list(svc):
            if sid:
                pid = int(svc.rsplit("-", 1)[-1])
                self.record_session_pane(pid, sid)
                self._dbus_map[sid] = ksid
